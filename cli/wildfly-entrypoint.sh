...
# Create module.xml directly (avoids CLI 'module add' syntax issues in WildFly 35)
...
# ── Step 3: Configure datasource using embed-server ──────────────────────────
# Check if ClinicDS is already configured in standalone-full.xml

if grep -q "ClinicDS" "$WILDFLY_HOME/standalone/configuration/standalone-full.xml" 2>/dev/null; then
    echo "[entrypoint] ClinicDS datasource already configured."
else
    echo "[entrypoint] Configuring datasource via embed-server..."
...
# ── Step 4: Clean stale deployment markers ───────────────────────────────────
# Only the EAR is mounted; remove any stale marker files so WildFly redeploys cleanly.
